...
ui <- fluidPage(
        ...
        fluidRow(
          ...
          column(4, wellPanel(
            uiOutput("checkboxesA"),uiOutput("checkboxesF")
          )),
...
server <- function(input, output) {
        ...
        observe({
                ...
                output$text <- renderText({
                        paste0('Selected City: ',citydata$city[which(citydata$CityIndex == click$id)[1]],', ',citydata$state[which(citydata$CityIndex == click$id)[1]])
                })
                output$graphs <- renderPlot({
                        # nested switch statement to retrieve the appropriate data from selected checkboxes
                        ...
                        big %>% filter(city %in% citydata$city[citydata$CityIndex == click$id]) %>%
                                ggplot(aes_string(x = "Date", y = metricchoice, col="city")) + 
                                geom_line() +
                        geom_line(data = big %>% filter(city %in% citydata$city[citydata$CityIndex == click$id]), aes_string(x = "Date", y = forecastchoice, col = "city")) #test city
                        
        })
                })
        
        output$checkboxesA <- renderUI({
